@extends('adminlte::page')

@section('title_prefix', 'Centro de Costos |')

@section('content_header')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Inicio</a></li>
            <li class="breadcrumb-item"><a href="{{ route('cost-centers.index') }}">Centros de Costos</a></li>
            <li class="breadcrumb-item active">#{{ $costCenterId }}</li>
            <li class="breadcrumb-item active">Detalles</li>
        </ol>
    </nav>
@endsection

@section('content')
    <h1 class="h4">Detalles de centro de costos</h1>
    <livewire:Catalogs.CostCenters.CostCenterShow :costCenterId="$costCenterId" />

    <hr class="mt-1">

    <h1 class="h4">Solicitudes con este centro de costos</h1>
    <livewire:Catalogs.CostCenters.RequestsTable :costCenterId="$costCenterId" />
@endsection
