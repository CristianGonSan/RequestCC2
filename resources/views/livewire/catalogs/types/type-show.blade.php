        ...
        <div class="card">
            <div class="card-body">
                <dl class="row mb-0">
                    ...
                    <dd class="col-sm-8 font-weight-bold mb-0">{{ $type->name }}</dd>
                </dl>
            </div>
        </div>

        <div class="mb-3">
            <a href="{{ route('types.edit', $type->id) }}" class="btn btn-outline-primary mr-1">
                <i class="fas fa-edit mr-1"></i> Editar
            </a>

            <x-livewire.loading-button label="Eliminar" theme="outline-danger" class="mr-1" icon="trash"
                wire:click="delete" wire:target="delete" wire:swal-delete="¿Eliminar este tipo?" :disabled="$isInUse"
                :title="$isInUse ? 'No se puede eliminar: el tipo está en uso' : ''" />

            <a href="{{ route('types.index') }}" class="btn btn-outline-secondary mr-1">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <dl class="row mb-0">
                    ...
                    <dd class="col-6" title="{{ $type->created_at->format('d/m/Y H:i') }}" data-toggle="tooltip"
                        data-placement="left">
                        {{ $type->created_at->diffForHumans() }}
                    </dd>

                    <dt class="col-6 text-muted">Actualizado</dt>
                    <dd class="col-6 mb-0" title="{{ $type->updated_at->format('d/m/Y H:i') }}" data-toggle="tooltip"
                        data-placement="left">
