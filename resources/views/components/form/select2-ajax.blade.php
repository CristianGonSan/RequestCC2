...
@push('js')
    <script>
        $(() => {
            ...
            const ajaxConfig = {
                ajax: {
                url: '{{ $config['url'] }}', // URL para la solicitud AJAX
                dataType: 'json',
                delay: {{ $config['delay'] ?? 250 }},
                data: function(params) {
                    return Object.assign({}, { search: params.term }, @json($config['params']));
                },
            ...
            const select2Config = Object.assign({}, baseConfig, ajaxConfig);
...
@once
    @push('css')
        <style type="text/css">
            {{-- SM size setup --}} .input-group-sm .select2-selection--single {
            ...
            {{-- LG size setup --}} .input-group-lg .select2-selection--single {
            ...
            {{-- Enhance the plugin to support readonly attribute --}} select[readonly].select2-hidden-accessible+.select2-container {
                pointer-events: none;
                touch-action: none;
            }
