...
class RequestCreate extends Component
{
    ...
    {
        return view('livewire.money-requests.users.requests-create', [
    ...
    public function save(): void
    {
        ...
        $moneyRequest = MoneyRequest::create($validated);

        MailManager::sendNewRequestNotification($moneyRequest);
        ...
        if ($this->createAnother) {
            ...
            redirect()->route('money-requests.show', $moneyRequest->id);
        }
    }

    private function loadCopy(int $copyFromId): void
    {
        $copyRequest = MoneyRequest::with(['costCenter:id,name,is_active', 'type:id,name,is_active'])->findOrFail($copyFromId);
