...
class RequestShow extends Component
{
    ...
    {
        return view('livewire.money-requests.accounting.request-show', [
    ...
    {
        $this->transitionStatus(MoneyRequestStatus::Paid);
    }

    public function cancelRequest(): void
    {
        $this->transitionStatus(MoneyRequestStatus::Cancelled);
    }

    private function transitionStatus(MoneyRequestStatus $status): void
    {
        $moneyRequest = $this->moneyRequest();

        if (! $moneyRequest->is_transfer || $moneyRequest->status->cannotChangeTo($status)) {
